...
# Determine base branch: allow override via BASE_BRANCH env; otherwise try to detect
# the current branch. If detection returns empty (detached HEAD or other cases),
# fall back to "main" to avoid trying to `git checkout ""` later.
