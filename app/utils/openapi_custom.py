...
from app.core.constants import VENUE_TAG_DESCRIPTION, API_TITLE, API_VERSION, API_DESCRIPTION
...
def generate_custom_openapi_schema(fastapi_app: FastAPI) -> dict[str, Any]:
    ...
    update_venue_tag_description(custom_schema)  # Update the schema with the "Venue" tag description
...
def custom_openapi(fastapi_app: FastAPI) -> dict[str, Any] | None:
    ...
    """
    if hasattr(fastapi_app.state, "openapi_schema") and fastapi_app.state.openapi_schema is not None:
